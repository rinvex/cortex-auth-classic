...
class EmailVerificationController extends AbstractController
{
    ...
    {
        return view('cortex/fort::frontarea.forms.verification-email-request');
    }
    ...
    public function verify(EmailVerificationProcessRequest $request)
    {
        ...
        switch ($result) {
            case EmailVerificationBrokerContract::EMAIL_VERIFIED:
                return intend([
                    'url' => $request->user($this->getGuard()) ? route('frontarea.account.settings') : route('frontarea.auth.login'),
                    'with' => ['success' => trans('cortex/fort::'.$result)],
                ]);
