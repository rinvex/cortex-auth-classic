...
class PhoneVerificationController extends AbstractController
{
    ...
    public function process(PhoneVerificationProcessRequest $request)
    {
        ...
        if (($user = $request->user(app('request.guard')) ?? app('cortex.auth.manager')->whereNotNull('phone')->where('phone', $request->input('phone'))->first()) && $this->isValidTwoFactorPhone($user, $request->input('token'))) {
            // Profile update
            $user->fill([
