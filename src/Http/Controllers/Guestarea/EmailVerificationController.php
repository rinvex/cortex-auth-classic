...
use Cortex\Fort\Http\Requests\Guestarea\EmailVerificationRequest;
use Cortex\Fort\Http\Requests\Guestarea\EmailVerificationSendRequest;
use Cortex\Fort\Http\Requests\Guestarea\EmailVerificationProcessRequest;
...
class EmailVerificationController extends AbstractController
{
     ...
     *
     * @param \Cortex\Fort\Http\Requests\Guestarea\EmailVerificationRequest $request
     *
     * @return \Illuminate\Http\Response|\Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
     */
    public function request(EmailVerificationRequest $request)
    {
        return view('cortex/fort::guestarea.forms.verification-email-request');
    }

    /**
     * Process the email verification request form.
     *
     * @param \Cortex\Fort\Http\Requests\Guestarea\EmailVerificationSendRequest $request
     *
     ...
     *
     * @param \Cortex\Fort\Http\Requests\Guestarea\EmailVerificationProcessRequest $request
     *
     * @return \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
     */
    public function verify(EmailVerificationProcessRequest $request)
    {
        ...
        switch ($result) {
            case EmailVerificationBrokerContract::EMAIL_VERIFIED:
                return intend([
                    'url' => $request->user($this->getGuard()) ? route('memberarea.account.settings') : route('guestarea.auth.login'),
                    'with' => ['success' => trans('cortex/fort::'.$result)],
                ]);
