...
{{-- Scripts --}}
@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Fort\Http\Requests\Tenantarea\EmailVerificationSendRequest::class)->selector('#tenantarea-verification-email-request-form') !!}
@endpush
                ...
                <section class="auth-form">
                    ...
                    {{ Form::open(['url' => route('tenantarea.verification.email.send'), 'id' => 'tenantarea-verification-email-request-form', 'role' => 'auth']) }}
                        ...
                        <div class="form-group has-feedback{{ $errors->has('email') ? ' has-error' : '' }}">
                            {{ Form::email('email', old('email', auth()->guest(request('guard')) ? '' : $currentUser->email), ['class' => 'form-control input-lg', 'placeholder' => trans('cortex/fort::common.email'), 'required' => 'required', 'autofocus' => 'autofocus']) }}
