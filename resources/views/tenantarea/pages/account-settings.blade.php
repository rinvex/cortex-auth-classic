...
@push('inline-scripts')
    {!! JsValidator::formRequest(Cortex\Auth\Http\Requests\Tenantarea\AccountSettingsRequest::class)->selector('#tenantarea-account-settings-form')->ignore('.skip-validation') !!}
        ...
        <div class="row profile">
            ...
            <div class="col-md-9">
                <div class="profile-content">
                    ...
                    {{ Form::model(app('request.user'), ['url' => route('tenantarea.account.settings.update'), 'id' => 'tenantarea-account-settings-form', 'files' => true]) }}
                                ...
                                <div class="form-group has-feedback{{ $errors->has('profile_picture') ? ' has-error' : '' }}">
                                    ...
                                    @if (app('request.user')->exists && app('request.user')->getMedia('profile_picture')->count())
                                        ...
                                        <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                           data-modal-action="{{ route('tenantarea.account.media.destroy', ['member' => app('request.user'), 'media' => app('request.user')->getFirstMedia('profile_picture')]) }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-group has-feedback{{ $errors->has('cover_photo') ? ' has-error' : '' }}">
                                    ...
                                    @if (app('request.user')->exists && app('request.user')->getMedia('cover_photo')->count())
                                        ...
                                        <a href="#" data-toggle="modal" data-target="#delete-confirmation"
                                           data-modal-action="{{ route('tenantarea.account.media.destroy', ['member' => app('request.user'), 'media' => app('request.user')->getFirstMedia('cover_photo')]) }}"
                                           data-modal-title="{{ trans('cortex/foundation::messages.delete_confirmation_title') }}"
                                           data-modal-button="<a href='#' class='btn btn-danger' data-form='delete' data-token='{{ csrf_token() }}'><i class='fa fa-trash-o'></i> {{ trans('cortex/foundation::common.delete') }}</a>"
